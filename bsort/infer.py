...
def run_inference(config_path: str, image_path: str, save_dir: str = "runs/inference") -> None:
    ...
    if cfg.get("use_wandb", False):
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            class_labels = results[0].names if isinstance(results[0].names, dict) else {i: n for i, n in enumerate(results[0].names)}

            wandb_boxes = {
                "predictions": {
                    "box_data": [
                        {
                            "position": {"minX": float(x1), "minY": float(y1), "maxX": float(x2), "maxY": float(y2)},
                            "class_id": int(cls_id),
                            "score": float(conf)
                        }
            ...
            wandb.log({"predictions": [wandb.Image(results[0].orig_img, boxes=wandb_boxes)]})
        wandb.finish()
